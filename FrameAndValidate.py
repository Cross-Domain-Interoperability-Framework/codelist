#!/usr/bin/env python3
"""
CDIF Codelist Profile JSON-LD Framing and Validation Script

Frames SKOS ConceptScheme documents according to the CDIF Codelist Profile
and validates them against the structured JSON Schema.

Usage:
    python FrameAndValidate.py <input-codelist.jsonld> [--output framed.json] [--validate] [--schema schema.json] [--frame frame.jsonld]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='CDIF Codelist Profile JSON-LD Framing and Validation Tool',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Frame a SKOS codelist and print output
  python FrameAndValidate.py my-codelist.jsonld

  # Frame with custom frame and save output
  python FrameAndValidate.py my-codelist.jsonld --frame CDIFCodelist-frame.jsonld -o framed.json

  # Validate against the CDIF Codelist Profile schema
  python FrameAndValidate.py my-codelist.jsonld -v --schema CDIFCodelistProfileStructuredSchema.json

  # Full workflow: frame and validate
  python FrameAndValidate.py my-codelist.jsonld --frame CDIFCodelist-frame.jsonld -o framed.json -v --schema CDIFCodelistProfileStructuredSchema.json
"""
    )
    ...
    parser.add_argument('--schema', default=str(SCRIPT_DIR / 'CDIFCodelistProfileStructuredSchema.json'),
                        help='Path to JSON Schema (default: CDIFCodelistProfileStructuredSchema.json)')
    parser.add_argument('--frame', default=str(SCRIPT_DIR / 'CDIFCodelist-frame.jsonld'),
                        help='Path to JSON-LD frame (default: CDIFCodelist-frame.jsonld)')
